    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          console.error("天気取得に失敗しました:", error);
        }
      },
      (error) => {
  ...
  return (
    <main className="min-h-screen bg-[#fdf2f6] pb-32 text-[#605D62]">
      <div className="mx-auto max-w-md px-4 pt-10">
        ...
        {/* ヘッダー */}
        <div className="mb-6">
          <p className="text-xs font-medium uppercase tracking-widest text-[#605D62]/40">
            Sakura Stylist
          </p>
          <h1 className={`mt-1 text-3xl font-bold text-[#605D62] ${notoSerifJP.className}`}>
            {userName}さんの
            <br />
            クローゼット
          </h1>
        ...
        {/* 天気カード（サイズ固定） */}
        <div className="mb-6 rounded-2xl bg-white px-5 py-4 ring-1 ring-[#E3F2FD]">
          {/* 上段 */}
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-[#605D62]/40">
                Today&apos;s Weather
              </p>
              <p className="mt-0.5 text-xs text-[#605D62]/60">
                {weather ? `${weather.cityName} · ${weather.description}` : "取得中..."}
              </p>
            </div>
            {weather ? (
              <img src={weather.iconUrl} alt={weather.description} className="h-12 w-12" />
            ) : (
              <div className="h-12 w-12 rounded-full bg-[#fdf2f6]" />
            )}
          </div>
          ...
          <div className="mt-2 flex items-end gap-3">
            {weather ? (
              <>
                <p className="text-3xl font-bold text-[#605D62]">{weather.temp}°</p>
                <p className="mb-1 text-xs text-[#605D62]/50">
                  {weather.tempMax}° / {weather.tempMin}°
                </p>
                {weather.isRainy && (
                  <div className="mb-1 flex items-center gap-1 rounded-full bg-[#E3F2FD] px-2 py-0.5">
                    <Drop size={11} color="#605D62" weight="fill" />
                    <span className="text-[10px] font-medium text-[#605D62]">雨</span>
                  </div>
                )}
                {weather.windSpeed >= 5 && (
                  <div className="mb-1 flex items-center gap-1 rounded-full bg-[#fdf2f6] px-2 py-0.5">
                    <Wind size={11} color="#605D62" />
                    <span className="text-[10px] font-medium text-[#605D62]">
              ...
              <div className="h-9 w-20 rounded-full bg-[#fdf2f6]" />
            )}
          </div>
        </div>
        ...
        {aiComment && (
          <div className="mb-6 flex items-start gap-3 rounded-2xl bg-white px-4 py-3 ring-1 ring-[#FCE4EC]">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#605D62]/50">
                他のスタイルもためしてみませんか？
              </p>
            </div>
          </div>
        ...
        >
          <div className="bg-gradient-to-br from-[#FCE4EC] via-[#fdf2f6] to-[#E3F2FD] px-5 py-5">
            <p className="text-xs font-medium tracking-wide text-[#605D62]/50">
              TODAY&apos;S PICK
            </p>
            <h2 className="mt-1 text-xl font-bold leading-snug text-[#605D62]">
              今日のコーデを
              <br />
              提案してもらう
            </h2>
            <p className="mt-2 text-xs text-[#605D62]/60">
              手持ち服からAIが最適なコーデを提案
            </p>
          </div>
        ...
        <div className="mb-6 rounded-2xl bg-white px-5 py-4 ring-1 ring-[#FCE4EC]">
          ...
          <div className="mb-1 flex items-end gap-2">
            <p className="text-2xl font-bold text-[#605D62]">
              {weeklyCount}
              <span className="ml-1 text-sm font-normal text-[#605D62]/60">回</span>
            </p>
        ...
        {recentLogs.length > 0 && (
          <div className="mb-6">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-xs font-semibold uppercase tracking-wide text-[#605D62]/40">
                Recent Logs
              </p>
              <Link href="/logs" className="text-xs text-[#605D62]/60">
                すべて見る →
              </Link>
            </div>
                  ...
                  <Link key={log.id} href="/logs" className="shrink-0">
                    <div className="relative h-24 w-24 overflow-hidden rounded-2xl ring-1 ring-[#FCE4EC]">
                      ...
                      <div className="absolute bottom-1 left-1 rounded-lg bg-white/80 px-1.5 py-0.5">
                        <p className="text-[10px] font-bold leading-none text-[#605D62]">
                          {date.getDate()}
                        </p>
                        <p className="text-[10px] leading-none text-[#605D62]/60">
                          {date.getMonth() + 1}月
                        </p>
                      </div>
                    </div>
        ...
        {/* 機能一覧 */}
        <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#605D62]/40">
          Features
        </p>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className={`text-sm font-bold text-[#605D62] ${notoSerifJP.className}`}>
                クローゼット分析
              </p>
            </div>
            <p className="text-xs leading-relaxed text-[#605D62]/50">
              足りないアイテムをAIが分析・提案
            </p>
          </Link>
          <Link
